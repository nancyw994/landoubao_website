...
export default function PlatformDropdown() {
  ...
  return (
    ...
    >
      <button className="flex items-center text-yellow-400 hover:text-white transition">
          ...
          <div className="space-y-4 mb-6">
            {platforms.filter(p => p.type === "官方账号").map((item, i) => (
              <div key={i} className="flex items-center gap-4">
                <img src={item.qr} alt={`${item.name} 二维码`} className={`w-16 h-16 border rounded ${item.status === 'unavailable' ? 'opacity-50 grayscale' : ''}`} />
                <div>
